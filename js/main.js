...
(function (html) {
    ...
    const ssLightbox = function () {
        ...
        folioLinks.forEach(function (link) {
            ...
            if (!modalElement) {
                console.warn('Modal not found for:', modalbox);
                modals.push(null); // keep index alignment
                return;
            }

            const instance = basicLightbox.create(modalElement, {
                onShow: function (instance) {
                    // Detect Escape key press to close modal
        ...
        folioLinks.forEach(function (link, index) {
            link.addEventListener("click", function (event) {
                ...
                // Only show modal if instance exists
                if (modals[index]) {
                    modals[index].show();
                } else {
                    console.warn('No modal instance found for this link.');
                }
            });
        });
...
skills.forEach(skill => {
    skill.addEventListener('mouseenter', () => {
        ...
        skill.style.setProperty('--skill-width', percent + '%');
    });
